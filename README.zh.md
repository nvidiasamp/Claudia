...
| 演示 | 内容 | 状态 |
|------|------|------|
| **核心演示** | 端到端：日语命令 → LLM 语义理解 → 机器人动作 | 已完成 |
| **语音管线** | USB 麦克风 → ASR → LLM → SafetyCompiler → 执行链路 | 已完成 |
| **终端控制** | 交互式启动器 → 键盘模式 → 文本命令 → 机器人执行 | 已完成 |
| **高级动作** | 高危模式切换 + 高级动作（拉伸、前跳、前空翻） | 已完成 |
| **多语言支持** | Whisper 语言切换 + Qwen2.5-7B 跨语言命令理解 | 计划中 |
| **安全系统** | 电量门控、站立前置条件、紧急停止 | 计划中 |
| **动作集锦** | 全部表演动作快剪蒙太奇 | 计划中 |
