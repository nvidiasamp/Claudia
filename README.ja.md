...
| デモ | 内容 | 状態 |
|------|------|------|
| **メインデモ** | エンドツーエンド：日本語コマンド → LLM 意味理解 → ロボット動作 | 完了 |
| **音声パイプライン** | USB マイク → ASR → LLM → SafetyCompiler → 実行チェーン | 完了 |
| **ターミナル操作** | ランチャー → キーボードモード → テキストコマンド → ロボット動作 | 完了 |
| **高度な動作** | 高リスクモード切替 + 高度な動作（ストレッチ、前ジャンプ、前宙） | 完了 |
| **多言語対応** | Whisper 言語切替 + Qwen2.5-7B 多言語コマンド理解 | 計画中 |
| **安全システム** | バッテリーゲーティング、起立前提条件、緊急停止 | 計画中 |
| **動作集** | 全パフォーマンス動作のクイックカットモンタージュ | 計画中 |
